...
async def main_krishnachecker():
    async with app:
            while True:
                print("Checking...")
                xxx_krish = f"**✨ <u>ᴡᴇʟᴄᴏᴍᴇ ᴛᴏ {(await app.get_chat(CHANNEL_ID)).title}**</u>\n\n**<u>💫  ʀᴇᴀʟ ᴛɪᴍᴇ ʙᴏᴛ's sᴛᴀᴛᴜs**</u>"
                for bot in BOT_LIST:
                    ...
                    try:
                        yyy_krish = await app.send_message(bot, "/start")
                        aaa = yyy_krish.id
                        await asyncio.sleep(30)
                        zzz_krish = app.get_chat_history(bot, limit = 1)
                        async for ccc in zzz_krish:
                            bbb = ccc.id
                        if aaa == bbb:
                ...
                await app.edit_message_text(int(CHANNEL_ID), MESSAGE_ID, xxx_krish)
                print(f"Last checked on: {last_update}")                
                await asyncio.sleep(2000)
                        
app.run(main_krishnachecker())
